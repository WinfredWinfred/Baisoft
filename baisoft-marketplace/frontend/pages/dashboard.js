...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Only fetch products if user has permission (admin, editor, approver)
        if (user?.role === 'admin' || user?.role === 'editor' || user?.role === 'approver') {
          try {
            const productsResponse = await apiService.get('/products/internal/');
  ...
  const canManageProducts = user?.role === 'admin' || user?.role === 'editor';
  ...
  return (
    <div className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Stats Grid - Only show if user has access */}
        {canManageProducts && (
        ...
        <div className="bg-white shadow rounded-lg border border-gray-200 p-6 mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">
            Welcome back, {userData?.username || user?.username}!
          </h2>
          <div className="space-y-2">
            ...
            <p className="text-sm text-gray-600">
              <span className="font-medium">Role:</span> <span className="capitalize">{userData?.role || user?.role}</span>
            </p>
            {(userData?.business || user?.business_name) && (
              <p className="text-sm text-gray-600">
                <span className="font-medium">Business:</span> {userData?.business?.name || user?.business_name}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white shadow rounded-lg border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              ...
              <div>
                <p className="font-medium text-gray-900">Public Marketplace</p>
                <p className="text-sm text-gray-500">View public products</p>
              </div>
            </Link>
          </div>
        </div>
